...
sub run
{
    ...
    if ( $ENV{SSH_DEVICE_IP} && $ENV{SSH_BIN_DIR} && $ENV{SSHPASS_BIN_DIR} ) {
      ...
      # Make sure that /work exists so that it can be mounted to
      my $cmd = "$SSHPASS_BIN -p '$ENV{SSH_DEVICE_PASS}' $SSH_BIN $ENV{SSH_DEVICE_USER}@$ENV{SSH_DEVICE_IP} \"mkdir /work\"";
      print "+ $cmd\n";
      system ($cmd);

      # mount own /work to device's /work
      $cmd = "$SSHPASS_BIN -p '$ENV{SSH_DEVICE_PASS}' $SSH_BIN $ENV{SSH_DEVICE_USER}@$ENV{SSH_DEVICE_IP} \"mount -t nfs $addr:/work /work\"";
      print "+ $cmd\n";
      system ($cmd);
    }
